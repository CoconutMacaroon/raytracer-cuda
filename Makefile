...
ifeq ($(TARGET_ARCH),armv7l)
  $(info >>> WARNING - simpleCUDA2GL is not supported on ARMv7 - waiving sample <<<)
  SAMPLE_ENABLED := 0
endif

# This sample is not supported on aarch64
ifeq ($(TARGET_ARCH),aarch64)
  $(info >>> WARNING - simpleCUDA2GL is not supported on aarch64 - waiving sample <<<)
  SAMPLE_ENABLED := 0
endif

# This sample is not supported on sbsa
ifeq ($(TARGET_ARCH),sbsa)
  $(info >>> WARNING - simpleCUDA2GL is not supported on sbsa - waiving sample <<<)
  SAMPLE_ENABLED := 0
endif
...
simpleCUDA2GL.o:simpleCUDA2GL.cu
	$(EXEC) $(NVCC) $(INCLUDES) $(ALL_CCFLAGS) $(GENCODE_FLAGS) -o $@ -c $<

simpleCUDA2GL: main.o simpleCUDA2GL.o
...
testrun: build
	$(EXEC) ./simpleCUDA2GL -file=ref_simpleCUDA2GL.ppm

clean:
	rm -f simpleCUDA2GL main.o simpleCUDA2GL.o
